...
{
    if [ "${prev}" == "--output" ] ; then
        complete -o default # Re-enable default file read
        COMPREPLY=()
...
_ipfs_swarm_disconnect()
{
    ...
    COMPREPLY=( $(compgen -W "${opts}" -- ${word}) )
    [[ $COMPREPLY = */ ]] && complete -o nospace -o filenames
}
...
_ipfs_hash_complete()
{
    ...
    [[ $COMPREPLY = */ ]] && complete -o nospace -o filenames # Removing whitespace after output & handle output as filenames. (Only printing the latest folder of files.)
    return 0
}
...
_ipfs_pinned_complete()
{
    ...
    if [[ ${#COMPREPLY[*]} -eq 1 ]]; then # Only one completion, remove pretty output
        ...
        [[ $COMPREPLY = */ ]] && complete -o nospace  # Removing whitespace after output
    fi
}
_ipfs_filesystem_complete()
{
